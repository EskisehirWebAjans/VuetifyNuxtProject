      ...
      <v-dialog
        ...
        <v-card>
          <v-toolbar
            ...
            <v-toolbar-title>Yeni Slider</v-toolbar-title>
          </v-toolbar>
          <v-card-text>
            <v-container>
              <v-row>
                  ...
                  <v-form
                    ...
                    <v-col
                      ...
                      <v-file-input
                        ...
                        placeholder="Slider Görseli Seç"
                ...
                <v-col
                  cols="12">
                  <v-text-field
                    label="Düğme Ana Başlık"
                    v-model="editedItem.subtitle_1"
                    clearable
                    outlined>
                  </v-text-field>
                </v-col>
                <v-col
                  cols="12">
                  <v-text-field
                    label="Düğme Alt Başlık"
                    v-model="editedItem.subtitle_2"
                    clearable
                    outlined>
                  </v-text-field>
                </v-col>
      ...
      <v-dialog
        ...
        <v-card>
          <v-toolbar
            ...
            <v-toolbar-title>Slider Sil</v-toolbar-title>
          </v-toolbar>
          <v-card-text>
            <h5 class="headline ma-8">Slider silinecek. Emin misiniz?</h5>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn
...
export default {
  ...
  async fetch({ store }) {
    await store.dispatch('slider/onLoadSliders')
  },
  methods: {
    ...mapActions('slider', {
      onLoadSliders: 'onLoadSliders',
    ...
    async onDeleteItemConfirm() {
      ...
      formData.append('data', slider)
      await this.onDeleteSlider({ slider: formData, index: this.editedIndex })
      this.onCloseDelete()
    },
    onPhotoSelect(photo) {
      if(photo) {
    ...
    async onSave() {
      ...
      if(this.valid) {
        if(this.editedIndex > -1) {
          ...
          await this.onUpdateSlider({ slider: formData, index: this.editedIndex })
        } else {
          const data = JSON.stringify({ ...this.editedItem, action: 'insert' })
          const formData = new FormData()
          formData.append('file', this.file, this.file.name)
          formData.append('data', data)
          await this.onSaveSlider(formData)
        }
          this.onClose()
      } else {
