      ...
      <v-dialog
        ...
        <v-card>
          <v-toolbar
            ...
            <v-toolbar-title>Duyuru Sil</v-toolbar-title>
          </v-toolbar>
          <v-card-text>
            <h5 class="headline ma-8">Duyuru silinecek. Emin misiniz?</h5>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn
...
export default {
  ...
  async fetch({ store }) {
    await store.dispatch('announce/onLoadAnnounces')
  },
  computed: {
    ...mapGetters('announce', [
      'announces',
    ...
    formTitle() {
      return this.editedIndex === -1 ? 'Yeni Duyuru' : 'Duyuru Düzenle'
    },
    labelErrors() {
      const errors = []
  ...
  methods: {
    ...mapActions('announce', {
      ...
      onUpdateAnnounce: 'onUpdateAnnounce',
    ...
    onDeleteItem(item) {
      this.editedIndex = this.announces.indexOf(item)
    ...
    async onDeleteItemConfirm() {
      ...
      await this.onDeleteAnnounce({ announce: formData, index: this.editedIndex })
      this.onCloseDelete()
    },
    onEditItem(item) {
      this.editedIndex = this.announces.indexOf(item)
    ...
    async onSave() {
      ...
      if(!this.$v.$invalid) {
        ...
        if(this.editedIndex > -1) {
          ...
          formData.append('data', data)
          await this.onUpdateAnnounce({ announce: formData, index: this.editedIndex })
          this.onClose()
        } else {
          let announce = {
          ...
          formData.append('file', this.file, this.file.name)
          formData.append('data', data)
          await this.onSaveAnnounce(formData)
          this.onClose()
        }
      } else {
