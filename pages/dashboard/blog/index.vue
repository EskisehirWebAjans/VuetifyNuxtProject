                ...
                <v-col
                  ...
                  <v-row
                    ...
                    <v-btn
                      ...
                      x-small
                      color="pink"
                      @click="onDeletePhoto(editedItem.image)">
      ...
      <v-dialog
        ...
        <v-card>
          <v-toolbar
            ...
            <v-toolbar-title>Blog Sil</v-toolbar-title>
          </v-toolbar>
          <v-card-text>
            <h5 class="headline ma-8">Blog yazısı silinecek. Emin misiniz?</h5>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn
...
export default {
  ...
  async fetch({ store }) {
    await store.dispatch('blog/onLoadBlogs')
  },
  computed: {
    ...mapGetters('blog', [
      'blogs',
    ...
    formTitle() {
      return this.editedIndex === -1 ? 'Yeni Blog' : 'Blog Düzenle'
    },
    labelErrors() {
      const errors = []
      if (!this.$v.editedItem.title.$dirty) return errors
      !this.$v.editedItem.title.required && errors.push('Blog başlığı gerekli!')
      return errors
    },
    contentErrors() {
      const errors = []
      if (!this.$v.editedItem.content.$dirty) return errors
      !this.$v.editedItem.content.required && errors.push('Blog içeriği gerekli!')
      return errors
    },
  },
  watch: {
  ...
  methods: {
    ...
    async onDeleteItemConfirm() {
      ...
      await this.onDeleteBlog({ blog: formData, index: this.editedIndex })
      this.onCloseDelete()
    },
    onEditItem(item) {
      this.editedIndex = this.blogs.indexOf(item)
    ...
    async onSave() {
      ...
      if(!this.$v.$invalid) {
        ...
        if(this.editedIndex > -1) {
          ...
          formData.append('data', data)
          await this.onUpdateBlog({ blog: formData, index: this.editedIndex })
        } else {
          const data = JSON.stringify({ ...this.editedItem,  action: 'insert' })
          const formData = new FormData()
          formData.append('file', this.file, this.file.name)
          formData.append('data', data)
          await this.onSaveBlog(formData)
        }
          this.onClose()
      } else {
